...
\newcommand{\pcv}{\pkg{purgedcv}}

%% -- Article metainformation --------------------------------------------------

\author{Evgenii Lazarev\\Independent Researcher}
\Plainauthor{Evgenii Lazarev}

\title{\pcv: Label-Aware Cross-Validation for Overlapping-Horizon Prediction in \proglang{Python}}
\Plaintitle{purgedcv: Label-Aware Cross-Validation for Overlapping-Horizon Prediction in Python}
\Shorttitle{\pcv: Label-Aware Cross-Validation}
...
overlapping-label time-series and panel data, and presents \pcv, a
\proglang{Python} implementation that exposes purging, embargoing,
walk-forward validation, group-purged folds, combinatorial purged
cross-validation, and diagnostic assertions through the \pkg{scikit-learn}
splitter protocol. A controlled experiment with an unpredictable target shows
...
generalization. The software, notebooks, tests, and benchmark scripts are open
source and make the validation choice auditable rather than implicit.
}

\Keywords{cross-validation, data leakage, time series, panel data, model validation, reproducible software, \proglang{Python}}
\Plainkeywords{cross-validation, data leakage, time series, panel data, model validation, reproducible software, Python}

\Address{
  Evgenii Lazarev\\
  Independent Researcher\\
  E-mail: \email{elazarev@gmail.com}\\
  ORCID: \url{https://orcid.org/0009-0000-1398-7842}
}

\Submitdate{2026-05-21}
...
an open \proglang{Python} implementation of purging, embargoing, walk-forward
validation, group-purged k-fold, CPCV path reconstruction, and leakage
diagnostics through the \pkg{scikit-learn} cross-validation interface
\citep{pedregosa2011sklearn,purgedcv2026}. Third, it reports reproducible
experiments that show both dramatic and undramatic outcomes: a synthetic task
where leakage fabricates strong skill, and a real smart-meter benchmark where
the larger issue is not temporal leakage but household-level generalization.
...
requirements executable. The package is written in \proglang{Python}, is MIT
licensed, and follows the \pkg{scikit-learn} splitter protocol, so the same
objects can be passed to \code{cross_val_score}, \code{GridSearchCV}, and
\code{Pipeline}. Runtime dependencies are intentionally small: \pkg{numpy},
\pkg{pandas}, \pkg{scikit-learn}, and \pkg{scipy}
...
package \citep{timeseriescv2018}.
\pkg{mlfinlab} is the best-known implementation associated with the financial
machine-learning literature, but it is distributed as a commercial product and
therefore cannot serve as a permissive dependency for open scientific software
\citep{mlfinlab2026}.
...
\pcv{} is therefore not differentiated by claiming new purging mathematics.
Its contribution is integration and auditability. Unlike fixed-gap splitters or
...
$H=20$, five outer folds, and seed 0. The overlap column is the mean fraction
of training rows whose label window overlaps any test label window, averaged
across folds.
...
\begin{center}
\small
\begin{tabular}{p{0.19\textwidth}p{0.35\textwidth}rrr}
\toprule
...
Power Networks and the London Datastore \citep{ukpn_lcl}. The prediction task
is half-hourly household electricity demand forecasting. Features include
calendar and lagged-load information, and the target is a forward-horizon mean.
The validation schemes compare pooled shuffled k-fold, blocked k-fold,
...
\href{https://github.com/eslazarev/purged-cross-validation}{project repository}
and distributed on PyPI as \pkg{purgedcv}. The repository is archived on Zenodo
under software concept DOI \doi{10.5281/zenodo.20312695}. The source
distribution contains the examples and benchmark tools; the wheel contains the
importable \pcv{} package.

The benchmark tables reported here were produced with \pcv{} 0.0.6,
\proglang{Python} 3.12.7, \pkg{numpy} 2.4.5, \pkg{pandas} 3.0.3,
\pkg{scikit-learn} 1.8.0, and \pkg{scipy} 1.17.1 on macOS 26.3.1
(\code{arm64}). The package supports \proglang{Python} 3.10 and later; runtime
dependency lower bounds are \pkg{numpy} 1.24, \pkg{pandas} 2.0,
\pkg{scikit-learn} 1.3, and \pkg{scipy} 1.10.
...
\path{paper/microbench_summary.md}. The full Low Carbon London
benchmark scanned 167,932,474 raw rows and ran 20 seeded 60-household
subsamples in 53.8 minutes on the author's local machine.
...
The last command expects the raw Low Carbon London CSV files to be present
locally. For faster checks, the repository includes end-to-end tests with
synthetic fixtures that exercise the same parser, feature builder, and
benchmark output format.

\section*{Generative AI disclosure}

Generative AI tools, including OpenAI Codex/ChatGPT from the GPT-5 family, were
used for code review, documentation drafting, and copy-editing. All design
decisions, AI-assisted changes, and outputs were reviewed and validated by the
author through unit, property, doctest, end-to-end, type-checking, and benchmark
tests.
...
\citep{devito2008airquality}, NOAA NCEI \citep{noaa_ghcnd}, the NASA
Prognostics Center of Excellence \citep{nasa_cmapss}, PhysioNet
...
methods implemented in \pcv{} are due to Lopez de Prado, Bailey, and
