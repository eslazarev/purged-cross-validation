...
% The standalone, always-compilable version is forecasting_selection_regret.tex.
%=================================================================
...
\abstract{Cross-validation does two jobs at once. It estimates out-of-sample error, and it ranks candidate models so that the top-ranked one is the model that ships. The second job is rarely audited. On panel data with repeated entities, shuffled k-fold can rank a model that memorizes entity identity above a simpler model that actually generalizes, because the test fold contains rows from the same entities as the training fold. The selected model then underperforms on entities it has never seen. We measure this effect on a real, partially predictable task: next-day household electricity demand from the UK Low Carbon London smart-meter trial. We select a model from a fixed eight-candidate grid (k-nearest neighbors, Ridge, and Random Forest) twice, once under naive shuffled k-fold and once under group-aware purged cross-validation, then deploy each selected model on twelve households held out entirely from selection. Naive cross-validation selects an unconstrained Random Forest that splits on a household-identifier feature and deploys at 2.638 kWh mean absolute error; group-aware cross-validation selects Ridge and deploys at 2.464 kWh, a 6.6\% reduction with higher coefficient of determination. Across 30 random partitions of the 60-household sample the group-aware selection deploys better in every partition (median gap 0.28 kWh, interquartile range 0.18 to 0.52). Measured against the best-deploying candidate in the grid, the group-aware selector's model-selection regret is essentially zero, while the naive selector's regret has a median of 0.28 kWh. The same selection is obtained with the ordinary GroupKFold splitter, and a leakage-aware encoding of the customer's average load reproduces the gap, so the effect is a property of group-aware validation rather than of any one feature or package. The practical message is that the cross-validation unit should match the deployment unit: forecasting for new customers calls for holding out customers, not rows.}
...
\code{examples/selection_regret_lcl_seeds.py}, the innocent-feature variant is
\code{examples/selection_regret_lcl_targetenc.py}, and a companion notebook,
\code{examples/selection_regret_lcl.ipynb}, reproduces the single-seed walk
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../.github/images/selection_regret_lcl_seeds.png}
...
Zenodo with DOI \doi{10.5281/zenodo.20312695}. The repository includes the cached
60-household subset, the experiment script
\code{examples/selection_regret_lcl_seeds.py}, the innocent-feature variant
\code{examples/selection_regret_lcl_targetenc.py}, the seed-sweep output, and the
notebook \code{examples/selection_regret_lcl.ipynb} needed to reproduce every
reported number and figure.}

\acknowledgments{During the preparation of this manuscript the author used large
language model tools for code review, refactoring suggestions, and copy-editing.
The author has reviewed and edited the output and takes full responsibility for
the content of this publication; no numerical claim was accepted without
executable verification against the experiment script.}
